...
import (
	...
	"github.com/ossf/scorecard/v5/internal/localization"
...
func (r *Result) AsString(writer io.Writer, checkDocs docChecks.Doc, opt *AsStringResultOption) error {
	...
	for i, row := range r.Checks {
		...
		x = append(x, localization.CheckName(row.Name, opt.Locale), row.Reason)
		if opt.Locale == options.LocaleZhCN {
			x = append(x, localization.Remediation(row.Name, opt.Locale))
		}
		if opt.Details {
